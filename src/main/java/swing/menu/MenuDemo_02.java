...
class MenuDemo_02 implements ActionListener {
    ...
    MenuDemo_02() {
        ...
        // Use default border layout. указать диспетчер поточной компоновки типа FlowLayout
        jfrm.setLayout(new FlowLayout());
        // Give the frame an initial size. задать исходные размеры фрейма
        jfrm.setSize(360, 200);

        // Terminate the program when the user closes the application. завершить прикладную программу, как только exit

        jfrm.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

        // Create a label that will display the menu selection. создать метку для отображения результатов выбора из меню
        jlab = new JLabel();

        // Create the menu bar. создать строку меню
        jmb = new JMenuBar();

        // Make the File menu. создать меню File
        makeFileMenu();
        ...
        // Make the Options menu. создать меню Options
        makeOptionsMenu();

        // Make the Help menu. создать меню Help
        makeHelpMenu();

        // Make the Edit popup menu. создать подменю Colors
        makeEditPUMenu();
    ...
    public void actionPerformed(ActionEvent e) {
        // Get the action command from the menu selection. получить команду действия из выбранного меню
        String comStr = e.getActionCommand();

        // If user chooses Exit, then exit the program. выйти из программы, если пользователь выберет пункт меню Exit
        if (comStr.equals("Exit")) System.exit(0);

        // Otherwise, display the selection. в противном случае отобразить результат выбора из меню
